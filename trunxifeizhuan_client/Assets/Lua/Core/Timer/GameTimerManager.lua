...
function GameTimerManager:StopTimerOnChangeScene()
	self:InternalStopTimerOnChangeScene(self.mScaleTimeList);
	self:InternalStopTimerOnChangeScene(self.mRealTimeList);
end

function GameTimerManager:DebugTimer()
	self:DebugTimerList(self.mScaleTimeList,"mScaleTimeList");
	self:DebugTimerList(self.mRealTimeList,"mRealTimeList");
end

function GameTimerManager:DebugTimerList(list,desc)
	...
	while node do 
		...
		if self.mChangeList[timer] == nil then
			print(desc,timer.mDebugTimerStr);
		end
	end
end

function GameTimerManager:InternalStopTimerOnChangeScene(list)
	local node = list.mFirst;
	local timer = nil;
	...
	while node do 
		timer = node;
		node = node.mLinkedNextNode;
		if timer.mStopOnChangeScene then
			list:Remove(timer);
		end
	end
end
...
function GameTimerManager:ExecuteChangeTimerList()
	...
	for key,timer in mpairs(changeList) do
		if timer.mRunning then
