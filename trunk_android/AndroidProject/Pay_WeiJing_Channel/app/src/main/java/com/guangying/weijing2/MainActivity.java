...
public class MainActivity extends QuickUnityPlayerproxyActivity {
    ...
    public  boolean IsWechatInstalled () {
       return MainActivity.isAvilible( mContext, "com.tencent.mm"); //return wxAPI.isWXAppInstalled();
    }

    //判断当前微信的版本是否支持API调用
    public  boolean IsWechatAppSupportAPI() {
        return  true;  //return wxAPI.isWXAppSupportAPI();
    }
    ...
    */
    ...
    public  void QuDaoRequestPermissions()
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            if (!permissionList.isEmpty())
            {
                ...
                Log.d("unitycall[permissions]:", (permissions.length + " "));
                this.activity.requestPermissions( permissions, 1);
            }
            else
            {
                UnityPlayer.UnitySendMessage("WWW_Set","onRecvPermissionsResult",  "1");
            }
        }
        else
        {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        Log.d("unitycall11", "onRequestPermissionsResult");
        switch (requestCode){
            case 1:
                if (grantResults.length>0){
                    for (int result:grantResults){
                        if (result!=PackageManager.PERMISSION_GRANTED){
                            Log.d("unitycall", "33333");
                            Toast.makeText(this,"请同意所以请求才能运行程序",Toast.LENGTH_SHORT).show();
                            UnityPlayer.UnitySendMessage("WWW_Set","onRecvPermissionsResult",  "0");
                            //finish();
                            return;
                        }

                        Log.d("unitycall", "4444444");
                        UnityPlayer.UnitySendMessage("WWW_Set","onRecvPermissionsResult",  "1");
                    }
                }else {
                    Log.d("unitycall", "55555");
