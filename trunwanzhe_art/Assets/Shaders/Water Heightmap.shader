...
Shader "Custom/Water Hightmap" {
	...
	Properties {
		...
		_MainTexTile ("Water Tile", float) =5.0 
		_HeightMapTile ("Heightmap Tile", float) = 3.5
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				// Background
				fixed4 background1 = tex2D(_MainTex, float2(_MainTexTile,_MainTexTile)*i.uv+(_MainTexRefraction*(distortion1+distortion2)));
				return background1;
			}
