...
namespace Assets.Editor
{
    ...
    {
        private static string[] sCheckPath = new string[] { "Assets/RawResources/UI" };
        private static int count = 0;

        [MenuItem("Assets/BundleEditor/CheckReferences", false, 0)]
        static public void CheckReferences()
        {
            ...
            if (assetGUIDs.Length > 0)
            {
                ...
                UnityEngine.Debug.LogError("开始检测" + fileName);

                count = 0;

                foreach (string str in sCheckPath)
                {
                    Check(fileName, new DirectoryInfo(str));
                }

                UnityEngine.Debug.LogError(string.Format("检测{0}引用完毕,共有{1}处引用", fileName, count));
            }
        }
        ...
        static void Check(string fileName, DirectoryInfo directoryInfo)
        {
            ...
            for (int i = 0, count = fileInfoList.Length; i < count; i++)
            {
                ...
                foreach (string path in dependPathList)
                {
                    ...
                    {
                        AddCount();
                        UnityEngine.Debug.LogError(string.Format("以下文件有引用{0}：{1}", fileName, fileInfo.FullName));
                        break;
                    }
                }
            }
            ...
            {
                Check(fileName, folderInfoList[j]);
            }
        }
