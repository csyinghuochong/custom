...
namespace Assets.Scripts.Com.Net
{
    public class NetSend : NetBase
    {
        ...
        public override void TraceNet()
        {
            ...
            Debug.Log(string.Format("[NetSend] mSendPack:{0},mConnectInitParam:{1},mListSendBuff.Count:{2}", null, mConnectInitParam, mListSendBuff.Count));
        }
            ...
            if (stream != null && stream.CanWrite && mListSendBuff.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, count = mThreadSendList.Count; i < count; i++)
                    {
                        ...
                        if (packet.mIsBack > 0)
                        {
                            if (MsgManager.sPackAction != null)
                                MsgManager.sPackAction(cmdID);
                        }
        ...
        {
            if (MsgManager.sWaitAction != null)
            {
                MsgManager.sWaitAction(packet.GetMsgID());
            }
