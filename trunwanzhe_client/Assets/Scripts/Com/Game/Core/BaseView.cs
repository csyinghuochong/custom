...
namespace Assets.Scripts.Com.Game.Core
{
    public class BaseViewParam
    {
        ...
        public bool replacePrevQueueWin = false; //替换之前触发banner的窗口,替换后点击返回键就不会返回之前窗口，例如创建联盟
        public bool forbidSound = false; //默认不屏蔽音效
        public bool hideUnderLayer = false;//显示时隐藏低层级的所有窗口，隐藏时显示低层级的所有窗口
        public bool rebuild = false;//界面显示状态下当值为true的话可以在切换回主界面时重构该界面销毁前的状态
        public bool HideRebuild = false;//界面隐藏状态下当值为true的话可以在切换回主界面时重构该界面销毁前的状态
        public bool hideNotice = false;//显示时默认不隐藏公告
        public bool speedUp = false;//打开界面时是否加速加载
        public bool forbidReplacePrevView = false;//是否关闭同一层级打开的界面
        ...
        private bool ShowUILoading(bool isShow)
        {
            if (mPreloadShowView == false && mViewParam.showUILoading)
            {
                ...
                else
                {
                    ...
                    GameTimer.ExecuteTotalFrames(10, null, delegate()
                    {
                        AssetLoader.Instance.AddLastCallBack(delegate()
                        {
                            //Debug.LogError("------------------------ShowUILoading:"+isShow);
