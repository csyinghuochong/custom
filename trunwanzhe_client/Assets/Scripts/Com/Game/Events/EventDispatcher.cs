...
namespace Assets.Scripts.Com.Game.Events
{
    public class EventDispatcher : Singleton<EventDispatcher>
    {
        ...
        private void DebugEvent(EventConstant typeCode, Delegate[] invocationList)
        {
            ...
            {
                if (typeCode != EventConstant.COMBAT_UPDATE_BLOOD
                    && typeCode != EventConstant.COMBAT_REFRESH_HERO_HP
                    && typeCode != EventConstant.COMBAT_UPDATE_TIME)
                    Debug.Log(string.Format("[EVENT] {0},invocationList:{1}", typeCode, invocationList == null ? 0 : invocationList.Length));
            }
        }
    }
}
